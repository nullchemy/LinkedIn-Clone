...
const installDep = async () => {
  ...
  await cp.exec('npm i node-cron shelljs json-format random-words', function(err, stdout, stderr) {
      if (err) {
      ...
      console.log('Dependencies installed: [ node-cron, shelljs, json-format, random-words]')
  });
}
